...
## Stale TPS / Active-Request Metrics After Deployment

Symptom: the dashboard shows a non-zero TPS value, a phantom active request, or a blank TTFT panel even though the GPU and proxy are idle.

Cause: the dashboard UI is only one half of the telemetry path. The live numbers come from the proxy tracker (`proxy/tracker.py`), and stale in-memory state can survive a dashboard-only deploy. A proxy restart or redeploy clears the stale tracker state and brings the idle stats back in sync.

Fix:
1. Deploy the dashboard changes with `python scripts/deploy_dashboard.py`.
